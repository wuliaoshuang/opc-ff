...
export default function AdminDashboardPage() {
  ...
  const expiringBindings = bindings.filter(
    (binding) =>
      ...
      Math.ceil(
        (new Date(binding.expiredAt).getTime() - Date.now()) / 86400000,
      ) <= 7,
  ).length;
  const draftTraining = trainingResources.filter(
    (item) => item.status === "draft",
  ).length;
  const missingAigcTemplates = (
    ["policy", "case", "opportunity"] as const
  ).filter(
    (type) =>
      !aigcTemplates.some(
  ...
  const pendingEvidence = redPacketTasks.filter(
  ...
  const workQueue = [
    ...
    },
    {
      ...
      tone: "text-green-700 bg-green-500/10",
    },
  ];
          ...
          <CardContent className="space-y-3">
            {funnelData.map((item) => {
              const pct = Math.round(
                (item.count / Math.max(totalLeads, 1)) * 100,
              );
              return (
                <div
                  key={item.stage}
                  className="grid grid-cols-[56px_1fr_42px] items-center gap-3"
                >
                  <span className="text-[12px] text-muted-foreground">
                    {item.stage}
                  </span>
                  <Progress
                    value={Math.max(pct, item.count > 0 ? 8 : 0)}
                    className="h-2"
                  />
                  <span className="text-right text-[12px] font-mono">
                    {item.count}
                  </span>
                </div>
              );
            ...
            {urgentProjects.length === 0 ? (
              ...
              urgentProjects.map((project) => (
                ...
                >
                  ...
                  <Badge
                    variant={project.isOverdue ? "destructive" : "outline"}
              ...
              <div key={partner.partnerId} className="rounded-xl border p-3">
                <div className="flex items-center justify-between">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="flex size-7 items-center justify-center rounded-full bg-muted text-[11px] font-bold">
                      {index + 1}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">
                  ...
                  <span
                    className={cn("font-bold", gradeColors[partner.rating])}
                  >
                    {partner.rating}
                  </span>
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2 text-[12px] text-muted-foreground">
                  <span>
                    成单{" "}
                    <b className="text-foreground">{partner.closedDeals}</b>
                  </span>
                  <span className="text-right">
                    项目{" "}
                    <b className="text-foreground">{partner.activeProjects}</b>
                  </span>
                </div>
              </div>
            ))}
