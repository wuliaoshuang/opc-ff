...
export default function SubPartnerPage() {
  ...
  const inviteCode = currentAccount
    ? getPartnerInviteCode(currentAccount)
    : user
      ? getPartnerInviteCode(user)
      : "";
  const inviteLink =
    typeof window === "undefined"
      ? ""
      : `${window.location.origin}${window.location.pathname}#/register?inviteCode=${encodeURIComponent(inviteCode)}`;
  const mySubs = sortByNewest(
    subPartners.filter((s) => s.parentId === user?.id),
    (sub) => sub.boundAt,
  );
  const active = mySubs.filter((s) => s.status === "active");
  const pendingInvites = accounts.filter(
    (account) =>
  ...
  const totalSubCommission = commissions
    .filter(
      (c) =>
        c.level === "secondary" &&
        mySubs.some((s) => s.id === c.partnerId) &&
        c.status === "settled",
    )
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <PageHeader
        ...
        action={
          <Button
            size="lg"
            className="gap-1.5"
            onClick={() => copyText(inviteCode, "邀请码")}
          >
            <Copy className="size-4" /> 复制邀请码
          </Button>
        }
      />
          ...
          <div className="rounded-xl border border-dashed bg-muted/30 p-3">
            ...
            <div className="mt-3 rounded-lg bg-background px-3 py-2">
              ...
              <div className="mt-1 flex items-center justify-between gap-2">
                <p className="truncate font-mono text-xl font-bold tracking-wider">
                  {inviteCode}
                </p>
                <Button
                  size="icon-sm"
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button
                ...
                variant="outline"
                className="gap-1.5"
                onClick={() => copyText(inviteLink, "注册链接")}
              >
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        ...
        <StatCard
          title="下级已结算收益"
          value={`¥${totalSubCommission.toLocaleString()}`}
          icon={TrendingUp}
        />
        <StatCard
          title="下级待结算"
          value={`¥${pendingSubCommission.toLocaleString()}`}
          icon={TrendingUp}
          changeType="up"
        />
      </div>
                  ...
                  {mySubs.map((sub) => (
                    ...
                    >
                      ...
                      <TableCell className="text-sm">
                        {formatListTime(sub.boundAt)}
                      </TableCell>
                      <TableCell>{sub.leads}</TableCell>
                      <TableCell>
                        {sub.activeProjects}/{sub.projects}
                      </TableCell>
                      <TableCell className="font-mono text-sm">
                        ¥{sub.totalCommission.toLocaleString()}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={
              ...
              >
                <CardContent className="p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    ...
                    <span>
                      项目 {sub.activeProjects}/{sub.projects}
                    </span>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] text-muted-foreground">
                      绑定于 {formatListTime(sub.boundAt)}
                    </span>
                    {sub.status === "active" && (
                      <Button
                        size="sm"
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-h-[90dvh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>手动补录二级合伙人</DialogTitle>
            <DialogDescription>
              用于线下已确认关系的历史数据补录；新下级建议使用邀请码注册
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={formRegion}
                onValueChange={(value) => value && setFormRegion(value)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="选择负责区域" />
                </SelectTrigger>
                <SelectContent>
                  {regions.map((r) => (
          ...
          <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {detail.status === "active" ? "活跃" : "已解绑"}
                </Badge>
                <Badge variant="outline" className="text-[10px]">
                  {detail.region}
                </Badge>
                <Badge variant="outline" className="text-[10px]">
                  上级: {detail.parentName}
                </Badge>
              </div>
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <div>
                  <span className="text-muted-foreground">线索数</span>
                  <p className="font-medium">{detail.leads}</p>
                </div>
                <div>
                  <span className="text-muted-foreground">项目数</span>
                  <p className="font-medium">
                    {detail.activeProjects}/{detail.projects}
                  </p>
                </div>
                <div>
                  <span className="text-muted-foreground">累计分佣</span>
                  <p className="font-medium">
                    ¥{detail.totalCommission.toLocaleString()}
                  </p>
                </div>
                <div>
                  <span className="text-muted-foreground">绑定日期</span>
                  <p className="font-medium">
                    {formatListTime(detail.boundAt)}
                  </p>
                </div>
              </div>
              <Separator />
              <div className="space-y-2">
                ...
                {commissions.filter(
                  (c) => c.partnerId === detail.id && c.level === "secondary",
                ).length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {sortByNewest(
                      commissions.filter(
                        (c) =>
                          c.partnerId === detail.id && c.level === "secondary",
                      ),
                      (c) => c.settledAt ?? c.month,
                    )
                      .slice(0, 5)
                      .map((c) => (
                        <div
                        ...
                        >
                          <div>
                            ...
                            <p className="text-muted-foreground">
                              {formatListTime(c.settledAt ?? c.month)} ·{" "}
                              {c.type === "short_term" ? "短期" : "长期"} ·{" "}
                              {c.commissionRate}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <Badge
                              variant={
                                c.status === "settled" ? "default" : "outline"
                              }
                              className="text-[10px]"
                            >
