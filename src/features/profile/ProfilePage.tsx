...
export default function ProfilePage() {
  ...
  const totalEarned = commissions.filter((c) => c.status === 'settled').reduce((sum, item) => sum + item.amount, 0)
  const paidRewards = redPacketTasks.filter((task) => task.status === 'paid').reduce((sum, task) => sum + task.amount, 0)
  const activeBindings = bindings.filter((binding) => binding.status === 'active' && binding.partnerId === user.id).length
  const mySubPartners = subPartners.filter((partner) => partner.parentId === user.id && partner.status === 'active').length

  const quickLinks = isAdmin
    ? [
        { label: '线索管理', sub: '归属与分配', icon: ShieldCheck, path: '/admin/leads' },
        { label: '合伙人', sub: '团队绩效', icon: Users2, path: '/admin/partners' },
        { label: '激励管理', sub: '红包与分佣', icon: Gift, path: '/admin/incentives' },
      ]
    : [
        { label: '客户绑定', sub: '查看保护期', icon: ShieldCheck, path: '/partner/binding' },
        { label: '二级合伙人', sub: '渠道团队', icon: Users2, path: '/partner/channel' },
        { label: '培训系统', sub: '资料与问答', icon: BookOpen, path: '/partner/training' },
        { label: 'AIGC 工具', sub: '政策/案例/机会', icon: Sparkles, path: '/partner/aigc' },
      ]
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <section className="overflow-hidden rounded-3xl border bg-zinc-950 p-5 text-white">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 space-y-1 text-[12px] text-white/60">
              <p className="flex items-center gap-1.5"><Phone className="size-3.5" />{user.phone}</p>
              <p className="flex items-center gap-1.5"><MapPin className="size-3.5" />{user.region} · {user.industry}</p>
            </div>
          </div>
        </div>
      ...
      {!isAdmin && (
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          ...
          <StatCard title="红包奖励" value={`¥${paidRewards.toLocaleString()}`} icon={Gift} />
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <p className="mt-1 font-medium">{isAdmin ? '平台管理员' : '城市合伙人'}</p>
            </div>
            <div>
      ...
      <Button variant="outline" className="h-11 w-full gap-2 text-destructive hover:text-destructive" onClick={handleLogout}>
