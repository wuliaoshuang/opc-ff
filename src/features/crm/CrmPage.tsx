...
import { Briefcase, Clock, ShieldCheck, AlertTriangle, CheckCircle2, ArrowRight, Plus, X, UserPlus, Video } from 'lucide-react'
...
const stageConfig: Record<ProjectStage, { label: string; color: string; icon: typeof Clock }> = {
  ...
  online_meeting: { label: '线上接洽', color: 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400', icon: ArrowRight },
...
export default function CrmPage() {
  ...
  const releasedProjects = projects
    .filter((p) => p.stage === 'exclusive' && p.exclusiveEnd && daysUntil(p.exclusiveEnd) < 0)
    .map((p) => p.companyName)

  const showBanner = releasedProjects.length > 0 && !dismissedBanner
  const needsContact = projects.filter((project) => project.stage === 'applied').length
  const needsMeeting = projects.filter((project) => project.stage === 'contact_filled').length
  const exclusiveCount = projects.filter((project) => project.stage === 'exclusive').length
  ...
  return (
    <div className="space-y-5 md:space-y-6">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-amber-500/30 bg-amber-50 p-4 dark:bg-amber-950/20">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-[12px] text-amber-700 dark:text-amber-400">
              以下项目排他期已到期，已自动释放至公海：
              <span className="font-medium ml-1">{releasedProjects.join('、')}</span>
            </p>
          </div>
          <button onClick={() => setDismissedBanner(true)} className="shrink-0 text-amber-600 hover:text-amber-800 dark:text-amber-500 dark:hover:text-amber-300">
            <X className="size-4" />
          </button>
        </div>
      )}
        ...
        {[
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === item.value ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-background/70',
            )}
          >
      ...
      {filtered.length === 0 ? (
        ...
        <>
              ...
              <Table>
                ...
                <TableBody>
                  {filtered.map((project) => {
                    ...
                    const deadline = project.stage === 'exclusive' && project.exclusiveEnd
          ...
          <div className="space-y-3 md:hidden">
            {filtered.map((project) => {
              const cfg = stageConfig[project.stage]
              const denominator = project.stage === 'applied' ? 30 : project.stage === 'contact_filled' ? 60 : project.stage === 'exclusive' ? 180 : 100
              const deadline = project.stage === 'exclusive' && project.exclusiveEnd
                ? `排他剩余 ${daysUntil(project.exclusiveEnd)} 天`
                : project.stage === 'applied'
                  ? `对接人截止 ${daysUntil(project.contactDeadline)} 天`
              ...
              return (
                <Card key={project.id} className="cursor-pointer rounded-2xl border-border/70 shadow-none" onClick={() => setSelectedProject(project)}>
                  <CardContent className="space-y-3 p-4">
                    ...
                    {(project.stage === 'applied' || project.stage === 'contact_filled' || project.stage === 'exclusive') && (
                      <Progress
                        value={project.stage === 'exclusive' && project.exclusiveEnd ? Math.max(0, Math.min(100, (daysUntil(project.exclusiveEnd) / denominator) * 100)) : project.stage === 'applied' ? 34 : 66}
                        className="h-1.5"
                      />
                    )}
      ...
      <Sheet open={!!selectedProject} onOpenChange={(open) => { if (!open) { setSelectedProject(null); setContactName(''); setContactRole(''); setContactPhone('') } }}>
        {selectedProject && (
            ...
            <div className="mt-4 space-y-4">
              <div className="flex gap-2">
                ...
                {selectedProject.isOverdue && <Badge variant="destructive">逾期</Badge>}
              </div>
              ...
              {selectedProject.contactPerson && (
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
                    ...
                    <span>信任度：{selectedProject.contactPerson.trustLevel}/10</span>
                  </div>
                </div>
              )}
              ...
              {selectedProject.stage === 'applied' && !selectedProject.contactPerson && (
                <>
                  ...
                  <div className="space-y-3">
                    <p className="text-[13px] font-medium flex items-center gap-1.5"><UserPlus className="size-3.5" /> 填写对接人信息</p>
                    <p className="text-[11px] text-muted-foreground">提交后自动推进至「对接人已填」阶段，请在1个月内完成</p>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select value={contactRole} onValueChange={(v) => { if (v) setContactRole(v) }}>
                          ...
                          <SelectContent>
                            {['设备科长', '能源管理部', '技术总监', '采购经理', '项目经理', '副总'].map((r) => (
              ...
              <p className="text-[11px] text-muted-foreground">确认线上接洽完成后，将直接进入180天排他保护期</p>
                  </div>
                </>
              )}
              ...
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <div><span className="text-muted-foreground">申请时间</span><p className="font-medium">{formatListTime(selectedProject.appliedAt)}</p></div>
                <div><span className="text-muted-foreground">最新跟进</span><p className="font-medium">{formatListTime(getProjectLatestTime(selectedProject))}</p></div>
              </div>
                ...
                <div className="space-y-2">
                  {selectedProject.followupLogs.map((log, i) => (
                    <div key={i} className="flex gap-3 text-sm">
                      ...
                      <div><span className="font-medium">{log.action}</span><span className="text-muted-foreground ml-2">{log.result}</span></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(selectedProject.stage === 'applied' && !selectedProject.contactPerson) && (
              <SheetFooter className="flex-row gap-2">
                <Button
                  ...
                  onClick={() => {
                    const contact = { name: contactName.trim(), role: contactRole, phone: contactPhone || '', trustLevel: 5, decisionLevel: 5 }
                    ...
                    toast.success(`对接人信息已提交，项目已推进至「对接人已填」`)
      ...
      <AddProjectDialog
        ...
        onSubmit={(project) => {
          ...
          if (conflict) {
            toast.warning(`项目已登记，但客户绑定由${conflict.partnerName}保护中，请先处理归属冲突`)
            return
          }
          const binding: CustomerBinding = {
            ...
            history: [
              ...
              ...(project.contactPerson
                ? [{ date: project.appliedAt, from: 'temporary' as const, to: 'locked' as const, action: `同步补全对接人：${project.contactPerson.name}（${project.contactPerson.role || '未填写角色'}）`, operator: user.name }]
                : []),
            ],
          }
          addBinding(binding)
