...
import { Search, FileText, Handshake, Bell, X, Save, Lightbulb, Target, MessageSquare, SlidersHorizontal } from 'lucide-react'
...
const regions = ['全部', '北京', '上海', '广州', '深圳', '成都', '杭州', '武汉', '南京', '长沙', '青岛', '大连', '珠海']
const resourceTags = ['人大', '政协', '协会', '商学院', '央企关系', '地方国企', '上市公司关系', '金融机构']
...
export default function LeadMiningPage() {
  ...
  const [prompt, setPrompt] = useState('优先寻找高耗能、扩建、招标或园区综合能源机会')
  ...
  const handleSearch = useCallback(() => {
    ...
    setTimeout(() => {
      let results = [...leads]
      if (industry && industry !== '全部') results = results.filter((l) => l.industry === industry)
      if (region && region !== '全部') results = results.filter((l) => l.region === region)
      if (keyword) {
        results = results.filter((l) =>
          [l.companyName, l.industry, l.projectInfo, l.businessInfo, l.newProjectProgress ?? '']
            .some((field) => field.includes(keyword)),
        )
      }
      results.sort((a, b) => {
        const resourceBoost = selectedResources.length > 0 ? 4 : 0
        const promptBoostA = prompt.includes(a.industry) || prompt.includes(a.region) ? 3 : 0
        const promptBoostB = prompt.includes(b.industry) || prompt.includes(b.region) ? 3 : 0
        return (b.aiMatchScore + resourceBoost + promptBoostB) - (a.aiMatchScore + resourceBoost + promptBoostA)
      })
      setSearchResults(results.slice(0, 12))
      setLoading(false)
    }, 1500)
  }, [leads, industry, region, keyword, prompt, selectedResources.length, setSearchResults])
  ...
  const handleApply = useCallback((id: string) => {
    ...
    if (conflict) {
      toast.error(`该客户已由${conflict.partnerName}绑定，当前阶段：${conflict.stage}`)
      return
    }

    applyLead(id, user.name)
    setAdminLeads(adminLeads.map((l) =>
      l.id === id ? { ...l, status: 'applied' as const, appliedBy: user.name, assignedPartner: user.name, updatedAt: new Date().toISOString().split('T')[0] } : l,
    ))
    ...
    if (lead) {
      ...
      addProject({
        ...
        isOverdue: false,
        source: 'lead',
        followupLogs: [{ date: now, action: '从AI线索池申请跟进', result: `线索匹配度 ${lead.aiMatchScore}%` }],
      })
      const binding: CustomerBinding = {
        ...
        linkedProjects: 1,
        history: [
          { date: now, from: 'released', to: 'temporary', action: '从AI线索池申请绑定', operator: user.name },
        ],
      }
      addBinding(binding)
    }
  }, [addBinding, addProject, adminLeads, applyLead, checkConflict, leads, setAdminLeads, user])
  ...
  const companyScore = (lead: PotentialLead) => Math.round((lead.isListed ? 40 : 20) + (lead.aiMatchScore > 85 ? 20 : 10))
  const energyScore = (lead: PotentialLead) => Math.round(lead.aiMatchScore * 0.3 + (lead.energyUsage === '极高' ? 25 : lead.energyUsage === '高' ? 18 : 10))
  const matchScore = (lead: PotentialLead) => Math.round(lead.aiMatchScore * 0.35)

  return (
    <div>
      <PageHeader title="AI 线索挖掘" description="设定关键词与资源标签，AI 自动匹配潜在项目并输出开发策略" />

      <section className="mb-4 overflow-hidden rounded-2xl border bg-zinc-950 p-4 text-white md:p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] text-white/55">线索雷达</p>
            <h2 className="mt-1 text-lg font-semibold tracking-tight">先筛资源，再申请保护</h2>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-xl bg-white/[0.07] p-3">
            ...
            <p className="mt-1 text-base font-semibold">{selectedResources.length}</p>
          </div>
          <div className="rounded-xl bg-white/[0.07] p-3">
            <p className="text-[10px] text-white/50">搜索结果</p>
            <p className="mt-1 text-base font-semibold">{searchResults.length}</p>
          </div>
          <div className="rounded-xl bg-white/[0.07] p-3">
        ...
        <div className="mb-4 flex items-center gap-3 rounded-lg border border-blue-500/20 bg-blue-50 px-4 py-3 dark:bg-blue-950/20">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-medium text-blue-900 dark:text-blue-200">
              本周 AI 新增 <span className="font-bold">{newCount}</span> 条潜在线索
            </p>
            <p className="text-[11px] text-blue-600 dark:text-blue-400">系统已根据您的行业标签自动更新推荐</p>
          </div>
          <button onClick={() => setShowBanner(false)} className="shrink-0 text-blue-400 hover:text-blue-600"><X className="size-4" /></button>
        </div>
      )}
      ...
      <Card className="mb-5 overflow-hidden rounded-2xl border-border/70 shadow-none md:mb-6">
        <CardContent className="p-0">
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">先输入客户画像，再交给 AI 排序</p>
                </div>
              </div>
              <Badge variant="outline" className="shrink-0 text-[10px]">{selectedResources.length} 个资源</Badge>
            </div>
          ...
          <div className="space-y-4 p-4 md:p-5">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select onValueChange={(v) => v && setIndustry(String(v))}>
                  <SelectTrigger className="h-10 rounded-xl"><SelectValue placeholder="选择行业" /></SelectTrigger>
                  <SelectContent>{industries.map((i) => <SelectItem key={i} value={i}>{i}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label className="text-[11px] font-medium text-muted-foreground">区域</Label>
                <Select onValueChange={(v) => v && setRegion(String(v))}>
                  <SelectTrigger className="h-10 rounded-xl"><SelectValue placeholder="选择区域" /></SelectTrigger>
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-medium text-muted-foreground">个性化提示词</Label>
              <Textarea
              ...
              />
              <p className="text-[11px] text-muted-foreground">提示词会影响线索排序与策略建议。</p>
            </div>

            <div className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <Label className="text-[11px] font-medium text-muted-foreground">资源标签</Label>
                <span className="text-[10px] text-muted-foreground">匹配您的圈层资源</span>
              </div>
            ...
            <Button onClick={handleSearch} disabled={loading} className="h-11 w-full rounded-xl">
              <Search className="mr-2 size-4" />{loading ? '挖掘中...' : '开始 AI 挖掘'}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {!loading && searched && searchResults.length === 0 && <EmptyState icon={Search} title="未找到匹配线索" description="请尝试调整搜索条件" />}
      {!loading && searchResults.length > 0 && (
        <>
          {/* Desktop Table */}
          <Card className="hidden md:block"><CardContent className="pt-4">
            <Table>
              <TableHeader><TableRow>
                <TableHead>公司名称</TableHead><TableHead>行业</TableHead><TableHead>区域</TableHead>
                <TableHead>上市</TableHead><TableHead className="w-[160px]">AI 评分</TableHead><TableHead>详情</TableHead><TableHead>操作</TableHead>
                ...
                {searchResults.map((lead) => {
                  ...
                  return (
                    <TableRow key={lead.id}>
                      ...
                      <TableCell>{lead.isListed ? <Badge>上市</Badge> : <Badge variant="outline">非上市</Badge>}</TableCell>
                      <TableCell>
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-muted-foreground w-10">企业</span>
                            <Progress value={cs} className="h-1.5 flex-1" />
                            <span className="text-[10px] font-mono w-6">{cs}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-muted-foreground w-10">能源</span>
                            <Progress value={es} className="h-1.5 flex-1" />
                            <span className="text-[10px] font-mono w-6">{es}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-muted-foreground w-10">匹配</span>
                            <Progress value={ms} className="h-1.5 flex-1" />
                            <span className="text-[10px] font-mono w-6">{ms}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" className="h-7 text-[11px] gap-1" onClick={() => openDetail(lead)}>
                            <FileText className="size-3" /> 详情
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 text-[11px] gap-1 text-primary" onClick={() => setPlanLead(lead)}>
                      ...
                      <TableCell>
                        {lead.status === 'available'
                          ? <Button size="sm" onClick={() => handleApply(lead.id)}>申请跟进</Button>
                          : <Button size="sm" disabled variant="outline">{statusLabels[lead.status]}</Button>}
                      </TableCell>
                    </TableRow>
                  )
                })}
          ...
          <div className="space-y-3 md:hidden">
            {searchResults.map((lead) => {
              ...
              const ms = matchScore(lead)
              return (
                <Card key={lead.id} className="rounded-2xl border-border/70 shadow-none">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="min-w-0">
                        <p className="font-medium text-sm truncate">{lead.companyName}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">{lead.industry} · {lead.region}</p>
                      </div>
                      {lead.isListed && <Badge className="shrink-0 text-[10px]">上市</Badge>}
                    </div>
                    <div className="space-y-1.5">
                      {[
                        ...
                        <div key={s.label} className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-mono w-5 text-right">{s.value}</span>
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-2 pt-1">
                      <Button variant="outline" size="sm" className="flex-1 h-8 text-[12px] gap-1" onClick={() => openDetail(lead)}>
                        <FileText className="size-3" /> 详情
                      </Button>
                      <Button variant="outline" size="sm" className="flex-1 h-8 text-[12px] gap-1 text-primary" onClick={() => setPlanLead(lead)}>
                        <Lightbulb className="size-3" /> 策略
                      </Button>
                      <Button size="sm" className="flex-1 h-8 text-[12px]" disabled={lead.status !== 'available'} onClick={() => handleApply(lead.id)}>
                        {lead.status === 'available' ? '申请' : statusLabels[lead.status]}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
      ...
      <Dialog open={!!detailLead} onOpenChange={(open) => { if (!open) setDetailLead(null) }}>
        {detailLead && (
          <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
            <DialogHeader><DialogTitle>{detailLead.companyName}</DialogTitle></DialogHeader>
            <div className="flex gap-3 text-[12px] text-muted-foreground">
              ...
              {detailLead.isListed && <Badge variant="outline" className="text-[10px] h-5">上市</Badge>}
            </div>
            <Separator />
            <div className="space-y-3">
                ...
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      detailTab === item.value ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground',
                    )}
                  >
                    {item.label}
                  </button>
              ...
              <textarea
                ...
                value={detailTab === 'project' ? editProjectInfo : editBusinessInfo}
                onChange={(e) => {
                  if (detailTab === 'project') setEditProjectInfo(e.target.value)
                  else setEditBusinessInfo(e.target.value)
                }}
              />
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setDetailLead(null)}>关闭</Button>
              <Button onClick={() => {
                updateLeadNotes(detailLead.id, { projectInfo: editProjectInfo, businessInfo: editBusinessInfo })
      ...
      <Sheet open={!!planLead} onOpenChange={(open) => { if (!open) setPlanLead(null) }}>
        {planLead && (() => {
          ...
          return (
            <SheetContent className="overflow-y-auto sm:max-w-xl">
              <SheetHeader>
                ...
                <p className="text-[13px] text-muted-foreground">{planLead.companyName} · {planLead.industry} · {planLead.region}</p>
              </SheetHeader>
              <div className="mt-6 space-y-6">
                <div className="space-y-2">
                  ...
                  <p className="text-[13px] text-muted-foreground leading-relaxed pl-9">{plan.opportunity}</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[13px] text-muted-foreground leading-relaxed pl-9">{plan.resourceAdvice}</p>
                </div>
                  ...
                  <div className="pl-9 space-y-2">
                    {plan.actionPlan.map((step, i) => (
                      <div key={i} className="flex gap-3 text-[13px]">
                        <Badge variant="outline" className="shrink-0 size-5 p-0 flex items-center justify-center text-[10px]">{i + 1}</Badge>
                ...
                <div className="space-y-2">
                  ...
                  <div className="pl-9 rounded-lg bg-muted/50 p-4">
                    <p className="text-[13px] text-muted-foreground leading-relaxed italic">"{plan.script}"</p>
                  </div>
                </div>

                <div className="pt-4 flex gap-3">
                  <Button variant="outline" className="flex-1" onClick={() => setPlanLead(null)}>关闭</Button>
                  <Button className="flex-1" onClick={() => { handleApply(planLead.id); setPlanLead(null) }}>
                    申请跟进此线索
                  </Button>
                </div>
              </div>
