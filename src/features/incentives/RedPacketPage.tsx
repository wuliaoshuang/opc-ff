...
import { Gift, Wallet, Upload, CheckCircle2, XCircle, Clock, Plus } from 'lucide-react'
...
const statusConfig: Record<RedPacketTask['status'], { label: string; variant: 'default' | 'secondary' | 'outline' | 'destructive'; icon?: typeof Clock }> = {
...
export default function RedPacketPage() {
  ...
  const getTaskTime = (task: RedPacketTask) => latestOf(task.evidence?.submittedAt, task.paidAt, task.claimedAt, task.createdAt, task.deadline)
  const sortedTasks = sortByNewest(tasks, getTaskTime)
  const filtered = filter === 'all' ? sortedTasks : sortedTasks.filter((t) => t.status === filter)
  const totalEarned = tasks.filter((t) => t.status === 'paid').reduce((s, t) => s + t.amount, 0)
  const available = tasks.filter((t) => t.status === 'available').length
  const executing = tasks.filter((t) => t.status === 'executing' || t.status === 'evidence_submitted').length

  const handleClaim = (task: RedPacketTask) => {
    if (task.projectName !== '通用任务') {
      const hasProject = projects.some((p) => p.companyName.includes(task.projectName) || task.projectName.includes(p.companyName))
      if (!hasProject) {
  ...
  const handleSubmitEvidence = (evidence: { images: string[]; description: string }) => {
  ...
  const handleCreateTask = () => {
    ...
    const task: IncentiveTask = {
      ...
      requirements: taskRequirements.trim() || '完成项目推进动作，并上传现场照片、沟通纪要或其他可审核凭证。',
    }
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border bg-zinc-950 p-4 text-white md:p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold tracking-tight">领取、执行、上传凭证一屏处理</h2>
          </div>
          <div className="shrink-0 text-right">
            <p className="text-2xl font-bold leading-none">¥{totalEarned.toLocaleString()}</p>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          ...
          <div className="rounded-xl bg-white/[0.07] p-3">
            ...
            <p className="mt-1 text-xl font-bold">{tasks.filter((t) => t.status === 'evidence_submitted' || t.status === 'reviewing').length}</p>
          </div>
        </div>
      </section>

      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        <StatCard title="已获得总额" value={`¥${totalEarned.toLocaleString()}`} icon={Wallet} />
        <StatCard title="可领取任务" value={available} icon={Gift} changeType="up" />
        ...
        {[
          ...
          <button
            ...
            className={cn(
              ...
              filter === item.value ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-background/70',
            )}
          >
            {item.label}
          </button>
      ...
      <>
                ...
                {filtered.map((task) => {
                  ...
                  return (
                    <TableRow key={task.id}>
                      ...
                      <TableCell className="font-mono font-medium text-amber-600">¥{task.amount.toLocaleString()}</TableCell>
                      <TableCell className="text-sm">{formatListTime(getTaskTime(task))}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          {task.status === 'available' && (
                            <Button size="sm" onClick={() => handleClaim(task)}>领取任务</Button>
                          )}
                          {task.status === 'executing' && (
                            <Button size="sm" variant="outline" className="gap-1" onClick={() => setUploadTask(task)}>
                          ...
                          {(task.status === 'evidence_submitted' || task.status === 'paid' || task.status === 'rejected') && (
                            <Button size="sm" variant="ghost" onClick={() => setDetail(task)}>查看详情</Button>
                          )}
                        </div>
                      </TableCell>
                    </TableRow>
        ...
        <div className="space-y-3 md:hidden">
          {filtered.map((task) => {
            ...
            const Icon = cfg.icon
            return (
              <Card key={task.id} className="rounded-2xl border-border/70 shadow-none">
                <CardContent className="space-y-3 p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-lg font-bold text-amber-600 shrink-0">¥{task.amount.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">更新 {formatListTime(getTaskTime(task))}</span>
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                    <div
                      ...
                      style={{
                        width: task.status === 'available' ? '18%' : task.status === 'executing' ? '52%' : task.status === 'paid' ? '100%' : task.status === 'rejected' ? '70%' : '78%',
                      }}
                    />
                  </div>
                  <div className="pt-1">
                    {task.status === 'available' && (
                      <Button size="sm" className="w-full" onClick={() => handleClaim(task)}>领取任务</Button>
                    )}
                    {task.status === 'executing' && (
                      <Button size="sm" variant="outline" className="w-full gap-1" onClick={() => setUploadTask(task)}>
                        <Upload className="size-3.5" /> 上传凭证
                      </Button>
                    )}
                    {(task.status === 'evidence_submitted' || task.status === 'paid' || task.status === 'rejected') && (
                      <div className="grid grid-cols-1 gap-2">
                        ...
                        <Button size="sm" variant="ghost" className="w-full" onClick={() => setDetail(task)}>查看详情</Button>
                      </div>
                    )}
                  </div>
                </CardContent>
          ...
          <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <div><span className="text-muted-foreground">关联项目：</span>{detail.projectName}</div>
                <div><span className="text-muted-foreground">奖励金额：</span><span className="font-mono font-medium text-amber-600">¥{detail.amount.toLocaleString()}</span></div>
                <div><span className="text-muted-foreground">截止日期：</span>{detail.deadline}</div>
                <div><span className="text-muted-foreground">更新时间：</span>{formatListTime(getTaskTime(detail))}</div>
                <div><span className="text-muted-foreground">状态：</span><Badge variant={statusConfig[detail.status].variant}>{statusConfig[detail.status].label}</Badge></div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-[12px] text-muted-foreground leading-relaxed">{detail.requirements}</p>
              </div>
              ...
              {detail.evidence && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-muted-foreground">提交时间：{detail.evidence.submittedAt}</p>
                  </div>
                </>
              )}
              ...
              {detail.reviewNote && (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className={`rounded-lg p-3 text-[12px] ${detail.status === 'rejected' ? 'bg-destructive/10 text-destructive' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400'}`}>
                      {detail.reviewNote}
                    </div>
                  </div>
                </>
      ...
      <UploadEvidenceDialog
        open={!!uploadTask}
        onOpenChange={(open) => { if (!open) setUploadTask(null) }}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <p className="text-[13px] text-muted-foreground">提交后进入后台激励管理审核，通过后自动发布到红包任务列表。</p>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-[12px]">任务名称</Label>
              <Input value={taskName} onChange={(e) => setTaskName(e.target.value)} placeholder="如：A类项目现场拜访激励" />
            </div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-[12px]">关联项目</Label>
                <Input value={taskProject} onChange={(e) => setTaskProject(e.target.value)} placeholder="输入项目或批量任务名称" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[12px]">奖励金额</Label>
                <Input value={taskAmount} onChange={(e) => setTaskAmount(e.target.value)} inputMode="numeric" placeholder="例如 2000" />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-[12px]">截止日期</Label>
              <Input value={taskDeadline} onChange={(e) => setTaskDeadline(e.target.value)} type="date" />
            </div>
            <div className="space-y-1.5">
